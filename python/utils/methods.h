...
template <typename Child> struct BaseMethod
{
...
#endif
        return PyMethodDef{const_cast<char*>(Child::name),
                           (PyCFunction)Child::run, Child::flags, Child::doc};
#pragma GCC diagnostic pop
    }

    static std::string docstring()
    {
        return build_method_doc(Child::name, Child::signature, Child::returns,
                                Child::summary, Child::doc);
    }
};
...
struct MethNoargs : public BaseMethod<Child>
{
    ...
    {
        PyErr_Format(PyExc_NotImplementedError, "method %s is not implemented",
                     Child::name);
        return nullptr;
    }
};
...
template <typename Child> struct ClassMethNoargs : public BaseMethod<Child>
{
...
template <typename Child> struct ClassMethKwargs : public BaseMethod<Child>
{
...
template <typename... METHODS> struct Methods
{
    ...
    PyMethodDef* as_py() { return const_cast<PyMethodDef*>(m_defs.data()); }
};
