...
struct DTableBase : public DTable
{
    ...
    DTableBase(const std::string& pathname) : m_pathname(pathname)
    {
        FILE* in = fopen(pathname.c_str(), "rt");
        if (in == NULL)
            error_system::throwf("opening D table file %s", pathname.c_str());
        ...
        unsigned nentries_check = 0; // Length of sequence advertised at the
        ...
        while (fgets(line, 200, in) != NULL)
        {
            ...
            {
                unsigned last_count =
                    static_cast<unsigned>(varcodes.size()) - begin;
                if (last_count != nentries_check)
                    error_parse::throwf(
                        pathname.c_str(), line_no,
                        "advertised number of expansion items (%u) does not "
                        "match the number of items found (%u)",
                        nentries_check, last_count);

                char* next = nullptr;
                nentries_check =
                    static_cast<unsigned>(strtoul(line + 7, &next, 10));
                if (nentries_check < 1)
                    throw error_parse(
                        pathname.c_str(), line_no,
                        "less than one entry advertised in the expansion");

                if (!varcodes.empty())
                    entries.push_back(Entry(
                        dcode, begin, static_cast<unsigned>(varcodes.size())));
            ...
            else if (strncmp(line, "           ", 11) == 0)
            {
                ...
                if (varcodes.empty())
                    throw error_parse(
                        pathname.c_str(), line_no,
                        "expansion line found before the first entry");
                // Check that we are not appending too many entries
                unsigned last_count =
                    static_cast<unsigned>(varcodes.size()) - begin;
                if (last_count == nentries_check)
                    error_parse::throwf(pathname.c_str(), line_no,
                                        "too many entries found (expected %u)",
                                        nentries_check);
        ...
        if (varcodes.empty())
            ...
            entries.push_back(
                Entry(dcode, begin, static_cast<unsigned>(varcodes.size())));

        // Check that the last entry is complete
        unsigned last_count = static_cast<unsigned>(varcodes.size()) - begin;
        if (last_count != nentries_check)
            error_parse::throwf(pathname.c_str(), line_no,
                                "advertised number of expansion items (%u) "
                                "does not match the number of items found (%u)",
                                nentries_check, last_count);
    }
    ...
    Opcodes query(Varcode var) const override
    {
        ...
        if (begin == -1 || entries[begin].code != var)
            ...
            return Opcodes(varcodes.data() + entries[begin].begin,
                           varcodes.data() + entries[begin].end);
    }
};
...
const DTable* DTable::load_bufr(const std::string& pathname)
{
    ...
    if (!tables)
        tables = new std::map<string, DTable*>;
