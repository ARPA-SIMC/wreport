    ...
    : bitmap(bitmap)
{
    ...
    //     *
    //     * From the specs it looks like bitmaps refer to all data that
    //     precedes the
    //     * C operator that defines or uses them, but from the data samples
    //     that we
    //     * have it look like when multiple bitmaps are present, they always
    //     refer
    ...
    if (b_cur == 0)
        throw error_consistency("data present bitmap has length 0");
    if (s_cur == 0)
        throw error_consistency(
            "data present bitmap is anchored at start of subset");
    ...
    while (true)
    {
        ...
        {
            if (s_cur == 0)
                throw error_consistency("bitmap refers to variables before the "
                                        "start of the subset");
            --s_cur;
        }
        ...
        if (s_cur == 0)
            throw error_consistency(
                "bitmap refers to variables before the start of the subset");
    }
...
void Bitmap::reuse() { iter = refs.rbegin(); }
...
unsigned Bitmaps::next()
{
    if (!current)
        throw error_consistency(
            "bitmap iteration requested when no bitmap is currently active");
...
void Bitmaps::reuse_last()
{
    ...
    //    if (!last)
    //        throw error_consistency("cannot reuse bitmap, because no bitmap is
    //        currently defined");
    delete current;
    current = last;
    last    = nullptr;
