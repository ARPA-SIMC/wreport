...
class Tests : public TestCase
{
    ...
    void declare_test(std::string fname,
                      std::function<void(const BufrBulletin&)> check_contents)
    {
        add_method(fname, [=]() {
            TestBufr test(fname);
...
void Tests::register_tests()
{
    ...
    add_method("bufr/corrupted.bufr", []() {
        ...
        catch (std::exception& e)
        {
            wassert(actual(e.what()).contains(
                "Only BUFR edition 2, 3, and 4 are supported (this message is "
                "edition 47)"));
        }
        wassert(actual(failed).istrue());
    });
    ...
    add_method("bufr/issue58.bufr", []() {
        // BUFR that declares a smaller master table version number than the one
        // it uses

        // Read the whole contents of the test file
        std::string raw = tests::slurpfile("bufr/issue58.bufr");

        wassert(actual((int)options::var_master_table_version_override) ==
        ...
        {
            wassert(actual(e.what()).contains(
                "variable 001110 not found in table"));
        }
        wassert(actual(failed).istrue());

        // Override the table
        options::LocalOverride lo(options::var_master_table_version_override,
                                  15);
    ...
    declare_test("bufr/crex-has-few-digits.bufr",
                 [](const BufrBulletin& msg) noexcept {
                     /*
                      * In this case, the ECMWF table has 12 bits for BUFR in
                      * Kelvin (up to 409.6) but 3 digits for CREX in Celsius
                      * (up to 99.0).  This means that BUFR can encode values
                      * too big to fit in CREX, and when it happens dba_var
                      * range checks kick in and abort decoding.
                      */
                 });
...
declare_test("bufr/tempforecast.bufr", [](const BufrBulletin& msg) {
    ...
    declare_test("bufr/C08032-toolong.bufr",
                 [](const BufrBulletin& msg) noexcept {});

    declare_test("bufr/synop-longname.bufr",
                 [](const BufrBulletin& msg) noexcept {
    ...
    declare_test("bufr/C04-B31021-1.bufr",
                 [](const BufrBulletin& msg) noexcept {
    ...
    declare_test("bufr/unparsable1.bufr",
                 [](const BufrBulletin& msg) noexcept {});

    declare_test("bufr/C04type21.bufr",
                 [](const BufrBulletin& msg) noexcept {});
    ...
    declare_test("bufr/A_ISMN02LFPW080000RRA_C_RJTD_20140808000319_100.bufr",
                 [](const BufrBulletin& msg) noexcept {});

    declare_test("bufr/bitmap-B33035.bufr",
                 [](const BufrBulletin& msg) noexcept {});
    ...
    declare_test_decode_fail("bufr/afl-src01flip1-pos10.bufr",
                             "but it must be at least 7");
    declare_test_decode_fail(
        "bufr/afl-src4824splice-rep8.bufr",
        "Optional section length is 3 but it must be at least 4");
    declare_test_decode_fail("bufr/short0.bufr",
                             "looking for section 0 of BUFR message");
    declare_test_decode_fail("bufr/short1.bufr",
                             "looking for section 0 of BUFR message");
    declare_test_decode_fail("bufr/short2.bufr",
                             "Only BUFR edition 2, 3, and 4 are supported");
    declare_test_decode_fail("bufr/short3.bufr",
                             "section 1 (Identification section) claims to end "
                             "past the end of the BUFR message");
